...
#import <Tools/SXBarButton.h>
#import <Tools/SXTabBar.h>
#import <Tools/SXAdManager.h>
...
- (void)viewWillAppear:(BOOL)animated {
    [self.tabBar.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (![obj isKindOfClass:[SXTabBar class]]) {
...
- (void)addChildViewControllers {
    UIViewController *main = [[Lothar shared] Main_aViewController];
    [self addChildViewController:main];
    SXEmptyViewController *read = [self createEmptyViewControllerWithImageNamed:@"125"];
    [self addChildViewController:read];
    SXEmptyViewController *video = [self createEmptyViewControllerWithImageNamed:@"121"];
    [self addChildViewController:video];
    SXEmptyViewController *find = [self createEmptyViewControllerWithImageNamed:@"122"];
    [self addChildViewController:find];
    SXEmptyViewController *mine = [self createEmptyViewControllerWithImageNamed:@"124"];
...
- (SXEmptyViewController *)createEmptyViewControllerWithImageNamed:(NSString *)imageName {
    SXEmptyViewController *viewController = [UIStoryboard storyboardWithName:@"Empty" bundle:nil].instantiateInitialViewController;
